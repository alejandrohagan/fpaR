...
make_segmentation <- function(df,id_col,kmeans_centers_init=3,kmeans_nstart=100,kmeans_iter.max=100,centers_grid_range=1:15,...) {

# logic checks

  ## check if id col is character

  if (!base::any(!base::is.character(df[[id_col]]) || !base::is.factor(df[[id_col]]))) {
  ...
  if(base::sum(purrr::map_lgl(mtcars, function(x) base::is.numeric(x) || base::is.integer(x))) < 2){
...
if(base::nrow(df) < kmeans_centers_init){
    stop(base::paste0("Error: number of rows in df (", base::nrow(df), ") is less than kmeans_centers_init (", kmeans_centers_init, "), please ensure df has enough rows for k-means clustering"))
  }


#checks for missing data
if (base::sum(purrr::map_lgl(df, ~ base::any(base::is.na(.)))) > 0) {
  ...
  } else if(base::max(centers_grid_range) > base::nrow(df)) {
...
# checks column types
  ...
  if(!base::all(purrr::map_lgl(df, ~ base::class(.) %in% c("numeric","integer","character","factor","ordered")))){
