...
#'
#'
#' calculate <- function(data, date_col, value_col, time_frame) {
...
#'   data <- data %>%
#'     mutate(floor_date = floor_date({{ date_col }}, time_frame))
#'
#'   # Summarize the data by the floored date
#'   summary_tbl <- data %>%
#'     group_by(floor_date) %>%
#'     summarise(inc = sum({{ value_col }}), .groups = "drop")
#'
#'   # Create a calendar table with all the dates in the specified time frame
#'   calendar_tbl <- tibble(
#'     floor_date = seq.Date(from = min(summary_tbl$floor_date), to = max(summary_tbl$floor_date), by = time_frame)
#'   )
#'
#'   # Perform a full join to ensure all time frames are represented
#'   full_tbl <- full_join(calendar_tbl, summary_tbl, by = "floor_date") %>%
#'     replace_na(list(inc = 0))
#'
#'   # Calculate the period-over-period changes
#'   full_tbl <- full_tbl %>%
#'     mutate(poop_change = inc - lag(inc, 1))
#'
#'   return(full_tbl)
#' }
#'
