...
contoso::sales |>
  ...
  calculate() |>
  collect() |>
  filter(missing_date_indicator == 0) |>                                  # <1>
  arrange(desc(mtd_gross_margin), store_key, product_key, date) |>        # <2>
  select(store_key, product_key, date, gross_margin, mtd_gross_margin) |>
  head(5)
```

1. Drop the zero-filled calendar rows so we are looking at real trading days.
2. Always sort before taking `head()`. A collected result has **no guaranteed row order** — the database returns rows in whatever order the query plan produced, so an unsorted `head()` gives you arbitrary rows that can change between runs.
