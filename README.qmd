...
`fpaR` is designed to seamlessly work with either tibbles or modern databases
...
speed and performance.^[I plan use [duckplyr](https://duckplyr.tidyverse.org/index.html) once it expands support for lubricate functions]
...
- **Optimized for speed and scale** your data is converted to a local [duckdb](https://github.com/duckdb/duckdb-r) database to scale your calculations
- **Instant clarity** as every function summarizes its transformation actions so that you can understand and validate the functions results 
...
If your data is a datable, the package will leverage [dbplyr](https://dbplyr.tidyverse.org/) to execute all the calculations.
...
3. Prints the sumamrized instructions the actions the functions performs to calculate the year over year analysis

## Why do we need this package when we have lubridate?

[Lubridate](https://lubridate.tidyverse.org/) is an excellent package and is heavily used by the package. The issue isn't lubridate but rather issues you may not be aware of in your package. 
...
would be missing `2024-01-02`, `2024-01-04`, and `2024-01-05` which will lead to
incorrect answers or trends.

To correct this, `fpaR` will complete your calendar for each group for the 
missing periods to ensure there are no missing periods when calculating trends.
...
-  This package leverages the [Contoso](https://github.com/alejandrohagan/contoso) package for its analysis. The contoso datasets are fictional business transaction of Contoso toy company which are helpful for business intelligence related analysis
